Shader "Amogus/AmogusDither"
{
    Properties
    {
        ...
        _DitherRangeLow("RangeLow", Float) = 0.1
        _DitherRangeHigh("RangeHigh", Float) = 0.4
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed3 frag(v2f i) : COLOR
            {
                ...
                float2 dUV = (i.uv * s - 0.7) * (_DitherScale * 0.7); // Reduce dithering frequency
                ...
                {
                    return col; // No dither for darker colors
                }

                float dither = 0.0;
                if (g <= lerp(_DitherRangeLow, _DitherRangeHigh, 0.1))
                {
                    dither = d1;
                }
                else if (g <= lerp(_DitherRangeLow, _DitherRangeHigh, 0.1))
                {
                    dither = d2;
                }
                else
                ...
                // Soften the dither effect
                return lerp(col, col * col * dither, _Strength * 0.5);
            }
            ENDCG
        }
    }
